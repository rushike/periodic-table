...
export function StandardPeriodicTable({elements} : StandardPeriodicTableProps) {
  ...
  return <div class="grid">{
    STANDARD_PERIODIC_TABLE_STRUCTURE.map(period => {
      return <div class="grid grid-cols-[repeat(18,calc(100%/18))] gap-x-0">{period.map(atomic_no => {
        const el = elements_map[atomic_no];
        console.log("rendering elment : ", elements_map[atomic_no]);
        if(el) return <div> <ElementCell element = {elements_map[atomic_no]}></ElementCell> </div>
        return <div></div>
      })}<br/></div>
    })
  }</div>
